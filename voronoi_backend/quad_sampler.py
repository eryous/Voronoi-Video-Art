...
class Quad:
    ...
    def __init__(self, img, x=0, y=0, min_area=64, error_rate=0.3, is_root=False):
        """Quad constructor 
        Keyword arguments:
        img -- the input image
    ...
    def __subdivide__(self):
        ...
        top_left = Quad(self.img[:child_height, :child_width, :], x=self.x,
                        y=self.y, min_area=self.min_area, error_rate=self.error_rate)
        top_right = Quad(self.img[:child_height, child_width:, :], x=self.x,
                         y=self.y + child_width, min_area=self.min_area, error_rate=self.error_rate)
        bottom_left = Quad(self.img[child_height:, :child_width, :], x=self.x +
                           child_height, y=self.y, min_area=self.min_area, error_rate=self.error_rate)
        bottom_right = Quad(self.img[child_height:, child_width:, :], x=self.x + child_height,
                            y=self.y + child_width, min_area=self.min_area, error_rate=self.error_rate)
        # Mark all children
